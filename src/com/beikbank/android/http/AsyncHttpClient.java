...
public class AsyncHttpClient {
	...
	public AsyncHttpClient() {
		...
		httpClient.addRequestInterceptor(new HttpRequestInterceptor() {
			public void process(HttpRequest request, HttpContext context) {
				...
				String cookie = BeikBankApplication.mSharedPref.getSharePrefString(SharePrefConstant.SETCOOKIE);
				request.setHeader("Cookie", cookie);
				//--------------------打印请求日志
				Utils.log(TAG, "request>>>>>>>>>>>>>>>--------------" );
				Header[] headers = request.getAllHeaders();
				for( int i = 0 ; i < request.getAllHeaders().length; i++){
					Utils.log(TAG, headers[i].getName() + ":" + headers[i].getValue() );
				}
				//--------------------
	...
	public void post(Context context, String url, RequestParams params, AsyncHttpResponseHandler responseHandler) {
		List<BasicNameValuePair> pairList = params.getParamsList();
		for( int i = 0  ; i < pairList.size() ; i++){
			Utils.log(TAG, url+"------------->>" + pairList.get(i).getName() + ":" +  pairList.get(i).getValue());
		}
