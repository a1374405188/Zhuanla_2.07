...
public class PurchaseSuccessActivity extends BaseActivity implements OnClickListener{
	...
	public void initView(){
		...
		PlayFund pf=(PlayFund)getIntent().getExtras().getSerializable(BeikBankConstant.INTENT_PURCHASESUCCESS);
		amount=getIntent().getStringExtra(BeikBankConstant.INTENT_PURCHASEAMOUNT);
		if(pf.status.equals("0")){
		...
		}else if(pf.status.equals("1")){
			textview_status_ok.setVisibility(View.GONE);
			textview_status_fail.setVisibility(View.GONE);
			textview_status_doing.setVisibility(View.VISIBLE);
			textview_status_ok.setText("确认中");			
			linear_status.setBackgroundResource(R.drawable.ic_purchase_doing);
		}else if(pf.status.equals("2")){
		...
		textview_ordernumber.setText(pf.orderNumber);
		textview_date.setText(pf.dealTime);
		//CardInfo cardInfo=CardInfoDao.getInstance(PurchaseSuccessActivity.this).getCardInfo();
	...
	public void backEvent(){
		Intent intent3 = new Intent(PurchaseSuccessActivity.this, HomeActivity3.class); 
